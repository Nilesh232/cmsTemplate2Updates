...
import classes from "../css/EventTemplate1Component.module.css";
...
import { updateEventsData } from "../../Services/Actions/actions";
...
function EventTemplate1Component(props) {
  const data = props.eventsPageData.events;
  const dummyObj = eventsObj;
  ...
  return (
    <>
          ...
          <div className={classes["text"]}>
            <div>
              ...
              <p className="breadcrumbs">
                ...
                <span>
                  events <i className="ion-ios-arrow-forward"></i>
                </span>
              </p>
            </div>
          </div>
      ...
      <div className={classes["main-content"]} style={{ marginTop: "3rem" }}>
        <div className="container">
          <section className={classes["posts-con"]}>
            {data.content.map((val, idx) => (
              <article className={editClass}>
                {props.isAuth === true ? (
                  <>
                    <div
                      className="edit-icon"
                ...
                <div className={classes["info"]}>
                  <h3>{val.title}</h3>
                  <p className={classes["info-line"]}>
                    <span className={classes["time"]}>{val.time}</span>
                    <span className={classes["place"]}>{val.location}</span>
                  </p>
                  {val.text.map((contentInfo) => (
                  <p className={classes["content"]}>{contentInfo}</p>
                    ))}
                </div>
              </article>
            ))}
...
const mapDispatchToProps = (dispatch) => ({
  updateData: (data, sectionName) =>
    dispatch(updateEventsData(data, sectionName)),
});

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(EventTemplate1Component);
